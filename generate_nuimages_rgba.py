...
dataroot = '/data/public/public_access/nuimages'

nuim_train = NuImages(dataroot=dataroot, version='v1.0-train', verbose=True, lazy=False)
nuim_val = NuImages(dataroot=dataroot, version='v1.0-val', verbose=True, lazy=False)
nuim_test = NuImages(dataroot=dataroot, version='v1.0-test', verbose=True, lazy=False)

value_dict = {}
for value, idx in enumerate(nuim_train.category):
    value_dict[idx['name']] = value + 1
...
for idx, nuim in zip(['/RGBA/train/', '/RGBA/val/', '/RGBA/test/'], [nuim_train, nuim_val, nuim_test]):
    for sample_idx in range(len(nuim.sample)):
        ...
        # get the image
        im = nuim.render_image(key_camera_token, annotation_type='none',
                  with_category=False, with_attributes=False, box_line_width=-1, render_scale=1, return_image=True)

        # get the semantic and instance masks
        semantic_mask, instance_mask = nuim.get_segmentation(key_camera_token)
        instance_mask = instance_mask.astype('float')

        # convert the semantic mask to the key values above
        unique_ids = np.unique(semantic_mask)
        for uid in unique_ids:
            if uid != 0:
                semantic_mask[semantic_mask == uid] = convert_seg_dict[uid]

        # scale the instance mask between 0 and 255
        if np.sum(instance_mask) != 0:
            instance_mask /= np.max(np.abs(instance_mask))
            instance_mask *= (255.0 / instance_mask.max())
            instance_mask = np.rint(instance_mask).clip(0, 255)

        # save each separately
        im.save(dataroot + idx + 'RGB/' + key_camera_token + '.png')

        sem = Image.fromarray(semantic_mask.astype('uint8'))
        sem.save(dataroot + idx + 'sem/' + key_camera_token + '.png')

        ins = Image.fromarray(instance_mask.astype('uint8'))
        ins.save(dataroot + idx + 'ins/' + key_camera_token + '.png')

        if (sample_idx % 500) == 0:
            print('Passing {} sample {}...'.format(idx, sample_idx))

    print('Finished generating nuimages {} RGBA data!'.format(idx))